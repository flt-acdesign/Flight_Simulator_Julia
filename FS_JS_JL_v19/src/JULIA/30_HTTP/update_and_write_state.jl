...
function update_state(req::HTTP.Request)
    ...
    try
        ...
        position = (  # Global axes
            x = float(data["x"]),
            y = float(data["y"]),
            z = float(data["z"])
        )
        velocity = ( # Global axes?
            x = float(data["vx"]),
            y = float(data["vy"]),
            z = float(data["vz"])
        )
        ...
        new_state = Runge_Kutta_4_integrator(s, force_control_inputs, moment_control_inputs, deltaTime, sim_constants)      
        ...
        # Prepare the response as a stringified JSON format
        response_body = JSON.json(response_dict)

        # Send the response with the new state to the Javascript client as a string containing a JSON data structure
        HTTP.Response(200, headers, response_body)
    catch e
