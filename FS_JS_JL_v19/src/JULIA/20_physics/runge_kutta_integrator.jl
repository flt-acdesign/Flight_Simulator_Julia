...
# This function is called by update_and_write_state
function Runge_Kutta_4_integrator(s, force_control_inputs, moment_control_inputs, deltaTime, sim_constants)
    
    # Runge-Kutta 4th order integration
    k1, force_total_k1, alpha_k1, beta_k1 = six_DOF_equations_of_motion(s, force_control_inputs, moment_control_inputs, sim_constants)
    k2, force_total_k2, alpha_k2, beta_k2 = six_DOF_equations_of_motion(s .+ (deltaTime / 2) .* k1, force_control_inputs, moment_control_inputs, sim_constants)
    k3, force_total_k3, alpha_k3, beta_k3 = six_DOF_equations_of_motion(s .+ (deltaTime / 2) .* k2, force_control_inputs, moment_control_inputs, sim_constants)
    k4, force_total_k4, alpha_k4, beta_k4 = six_DOF_equations_of_motion(s .+ deltaTime .* k3, force_control_inputs, moment_control_inputs, sim_constants)
